...
def detect_language(text):
    ...
    if any(word in text.lower() for word in ['today', 'tomorrow', 'next week', 'next month']):
        return 'en'
    elif any(word in text.lower() for word in ['heute', 'morgen', 'nächste woche', 'nächsten monat','ist','ich', 'möchte']):
        return 'de'
...
def extract_name_date_time(user_input):
    ...
    for ent in doc.ents:
        if (ent.label_ == "PERSON" or ent.label_ == "GPE") and not name:
            ...
            if language == 'de':
                raise ValueError # FUTURE DEVELOPMENT MULTI LANGUAGE
    ...
    if date:
        ...
        if date.lower() in ['today','tomorrow','next week','next month','hari ini','heute']:
            # get language
            if language == 'de':
    ...
    if name or (date and start_time and end_time):
...
def find_available_slots(message):
    ...
    if 'today' in message.lower():
        ...
        start_date = end_date = today + timedelta(days=6)
        # end_date = start_date + timedelta(days=6)
    elif 'next month' in message.lower():
        start_date = end_date = today + timedelta(days=30)
        # end_date = start_date + timedelta(days=30)
    else:
        raise ("Please specify 'today' or 'next week' or 'next month' in your message.")
...
def chat():
    ...
    if 'book' in user_input.lower() or 'bestellen' in user_input.lower() or 'pesan' in user_input.lower():
        try:
        ...
        try:
            ...
            global temp_old_name, temp_old_date_str, temp_old_start_time_str
            temp_old_name = name
            temp_old_date_str = old_date_str
            temp_old_start_time_str = old_start_time_str
            
            response = "Please provide the new date and time for rescheduling your appointment."
        except ValueError as e:
            response = str(e)
    
    elif 'new date' in user_input.lower():
        try:
            ...
            if check_availability(new_date_str, new_start_time_str, new_end_time_str):
                reschedule_appointment(temp_old_name, temp_old_date_str, temp_old_start_time_str, new_date_str, new_start_time_str, new_end_time_str)
                response = f"Your appointment has been rescheduled {temp_old_name} to {new_date_str} from {new_start_time_str} to {new_end_time_str}"
            else:
